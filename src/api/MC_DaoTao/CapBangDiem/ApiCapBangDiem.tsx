...
export interface IDataCapBangDiem {
  DenTiet: string;
  GioiTinh: boolean;
  HoDem: string;
  IDDot: number;
...
export interface IDataPostCapBangDiem {
...
export interface IGetKetQuaCapBangDiem {
  code: number;
  arrayLength: number;
  message: string;
  rowsAffected: number[];
  body: IDataCapBangDiem[];
}

const path = {
  postYeuCauCapBangDiem: '/api/SP_MC_DT_CapBangDiem_TiepNhan/Add_Para',
};

const dispatch = store.dispatch;

function postYeuCauCapBangDiem(
  body: IDataPostCapBangDiem,
): Promise<IGetKetQuaCapBangDiem> {
  return fetcher(
    {url: path.postYeuCauCapBangDiem, method: 'post', data: body},
    dispatch,
    {
      displayError: true,
    },
